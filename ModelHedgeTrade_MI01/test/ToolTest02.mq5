...
int OnInit()
   ...
   */
   ...
   //+------------------------------------------------------------------+
   //makeOrders(300);   
   //printf("orders count:" + PositionsTotal());
   ...
   for(int i=0;i<symbolCount;i++){
      double symbolPointValue=getSymbolTradeTickValue(SYMBOL_LIST[i]);
      printf(SYMBOL_LIST[i] + "----" + symbolPointValue);
   }
...
void OnTick()
{
   ...
   /*
    ...
    if((TimeCurrent()-preTime)>100){
    
       double pointValue;
       //string symbol="EURUSD";
       if (!SymbolInfoDouble(symbol, SYMBOL_TRADE_TICK_VALUE, pointValue))
       {
           Print("无法获取每点的货币价值。");           
       }
       printf(symbol + "--pointValue:" + pointValue);
       preTime=TimeCurrent();
    }
    */
}
...
double getSymbolTradeTickValue(string symbol){

       double pointValue;
       //string symbol="EURUSD";
       if (!SymbolInfoDouble(symbol+symbolSuffix, SYMBOL_TRADE_TICK_VALUE, pointValue))
       {
           return 0;
       }
       return pointValue;
