...
class CPriceChlCom{
   private:
      ...
      int    Handles[SYMBOL_MAX_COUNT][200];                   // 保存指标句柄
...
void  CPriceChlCom::makeIndicatorData(int symbolIndex)
  {
   ...
   string symbol=comFunc.addSuffix(SYMBOL_LIST[symbolIndex]);      
   double curATR=comFunc2.GetATR(symbol,0,this.channelTimeFrame,24);
   double curStdDev=comFunc2.GetStdDev(symbol,0,this.channelTimeFrame,24);
   ...
   adjustPeriod[0]=(int)(comFunc2.mapValue(sumAtrStdDev,0,10,16,2)); 
   adjustPeriod[1]=(int)(comFunc2.mapValue(sumAtrStdDev,0,10,24,6)); 
   adjustPeriod[2]=(int)(comFunc2.mapValue(sumAtrStdDev,0,10,32,9)); 
   adjustPeriod[3]=(int)(comFunc2.mapValue(sumAtrStdDev,0,10,48,12)); 
   adjustPeriod[4]=(int)(comFunc2.mapValue(sumAtrStdDev,0,10,72,16)); 
   adjustPeriod[5]=(int)(comFunc2.mapValue(sumAtrStdDev,0,10,96,24)); 
   adjustPeriod[6]=(int)(comFunc2.mapValue(sumAtrStdDev,0,10,180,32)); 
   ...
   /*
   double maxSumRate=logData.getCheckTimeDiffNValue("maxSumRate",3600*24*30,0);
   double minSumRate=logData.getCheckTimeDiffNValue("minSumRate",3600*24*30,1000);
   if(this.sumRate>maxSumRate){
      logData.addCheckTimeDiffNValue("maxSumRate",this.sumRate);
   }
   if(this.sumRate<minSumRate){
      logData.addCheckTimeDiffNValue("minSumRate",this.sumRate);
   }     
   
   logData.addLine("<sumRate>" + StringFormat("%.2f",this.sumRate)
                     + "<strengthSum1>" + StringFormat("%.2f",this.sumChlHeight)
                     + "<strengthSum2>" + StringFormat("%.2f",strengthSumPips)
                     + "<edgeRate>" + StringFormat("%.2f",edgeRate)
                     + "<strengthRate>" + StringFormat("%.2f",strengthRate)
                     + "<rulePips>" + StringFormat("%.2f",this.strengthUnitPips)
                     + "<maxS>" + StringFormat("%.2f",maxSumRate)
                     + "<minS>" + StringFormat("%.2f",minSumRate)
                     ); //---logData test     
   */
}
