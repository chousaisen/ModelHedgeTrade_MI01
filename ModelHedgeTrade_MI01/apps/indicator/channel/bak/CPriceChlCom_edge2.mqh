...
void  CPriceChlCom::makeIndicatorData(int symbolIndex)
  {
   ...
   double pre_channel_upper=0,pre_channel_lower=0;
   // 忽略最大周期64本身
   for(int p = periodCount - 1; p > 0; p--){      
      ...
      if(p == (periodCount - 1)){
         pre_channel_upper=PeriodUpper[0];
         pre_channel_lower=PeriodLower[0];      
         continue;
      }

      // 归一化计算靠近上边缘和下边缘的贴合程度
      double channel_distance = PeriodUpper[0] - PeriodLower[0];
      double channel_up_edge_diff=pre_channel_upper-PeriodLower[0];
      double channel_up_edge_diff2=PeriodUpper[0]-pre_channel_upper;
      double channel_lower_edge_diff=PeriodUpper[0]-pre_channel_lower;
      double channel_lower_edge_diff2=PeriodLower[0]-pre_channel_lower;
   ...
   if(rkeeLog.debugPeriod(9112,60)){     
      ...
      string logTemp ="<adjustPeriod>" + adjustPeriod[0]
                        + "-" + adjustPeriod[1]
                        + "-" + adjustPeriod[2]
                        + "-" + adjustPeriod[3]
                        + "-" + adjustPeriod[4]
